...
<div style="position: inherit;">
    <div class="navigation">
        ...
        <?foreach($arResult['SECTIONS'] as $section):?>
            <li <?if(!isset($section['CHILDS'])):?>onclick="showDivNeeded(<?=$section['ID']?>)"<?endif;?>>
                ...
                <?if(isset($section['CHILDS'])):?>
                    <ul class="submenu">
                        <?foreach($section['CHILDS'] as $child):?>
                            <li class="<?if(isset($child['CHILDS'])):?>thirdMenuMain" <?else:?> onclick="showDivNeeded(<?=$child['ID']?>)"<?endif;?>>
                                <a><?=$child['NAME']?> <?=!isset($child['CHILDS'])?'('.(isset($child['CNT_ORDER'])? $child['CNT_ORDER']: 0).')':''?></a>
                                <?if(isset($child['CHILDS'])):?>
                                    <ul class="thirdmenu">
                                        <?foreach($child['CHILDS'] as $child3):?>
                                            <li onclick="showDivNeeded(<?=$child3['ID']?>)">
    ...
    <?foreach($arResult['PRODUCTS'] as $product):?>
        ...
        <div class="product">
            ...
            <a href="<?=$product['DETAIL_PAGE_URL']?>"><?=$product['NAME']?> (<?=$product['QUANTITY']?> шт.)</a>
